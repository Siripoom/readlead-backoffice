...
async function main() {
  ...
  const demoWorks = [
    { id: 'creator-demo-novel', type: 'novel' as const, title: 'แสงดาวเหนือหอคอย', category: 'fantasy', tagline: 'คำสัญญาหนึ่งคืนเปลี่ยนชะตาของทั้งอาณาจักร', synopsis: 'เรื่องราวของนักพยากรณ์ฝึกหัดที่ต้องตามหาดาวดวงสุดท้ายก่อนคืนจันทรคราส', tags: ['แฟนตาซี', 'ผจญภัย'], views: 18420, coins: 3560, shelfCount: 942, dailyVotes: 128, monthlyVotes: 732, reviewCount: 2, commentCount: 116 },
    { id: 'creator-demo-manga', type: 'manga' as const, title: 'ร้านชาในวันฝนพรำ', category: 'romance', tagline: 'ทุกแก้วชามีเรื่องราวที่ยังเล่าไม่จบ', synopsis: 'เว็บตูนอบอุ่นหัวใจเกี่ยวกับร้านชาเล็ก ๆ และลูกค้าที่แวะเวียนมาพร้อมความทรงจำ', tags: ['โรแมนซ์', 'ฮีลใจ'], views: 9320, coins: 1480, shelfCount: 514, dailyVotes: 62, monthlyVotes: 348, reviewCount: 2, commentCount: 75 },
    { id: 'creator-demo-audio', type: 'audiobook' as const, title: 'เสียงจากปลายฤดู', category: 'drama', tagline: 'จดหมายเสียงที่เดินทางข้ามฤดูกาล', synopsis: 'หนังสือเสียงดราม่าที่เล่าผ่านจดหมายและเสียงบันทึกของคนสองคน', tags: ['ดราม่า', 'หนังสือเสียง'], views: 6480, coins: 920, shelfCount: 286, dailyVotes: 41, monthlyVotes: 205, reviewCount: 2, commentCount: 31 },
  ]
  for (const [index, work] of demoWorks.entries()) {
    const narrationType = work.type === 'audiobook' ? 'human' as const : null
    await prisma.creatorWork.upsert({ where: { id: work.id }, update: { ...work, creatorId: demoCreator.id, origin: 'original', status: 'published', rating: 'general', creationMethod: 'self_written', narrationType, seriesStatus: 'ongoing', publishedAt: new Date(`2026-07-0${index + 2}T08:00:00Z`) }, create: { ...work, creatorId: demoCreator.id, origin: 'original', status: 'published', rating: 'general', creationMethod: 'self_written', narrationType, seriesStatus: 'ongoing', publishedAt: new Date(`2026-07-0${index + 2}T08:00:00Z`) } })
    ...
    const demoReviews = [
      { id: `${work.id}-review-u1`, userId: 'u1', rating: 5, body: 'เล่าเรื่องสนุกมาก ตัวละครมีเสน่ห์และอยากติดตามตอนต่อไป', recommended: true, spoiler: false },
      { id: `${work.id}-review-u2`, userId: 'u2', rating: 4, body: 'จังหวะเรื่องดี อ่านเพลิน และมีรายละเอียดที่น่าสนใจ', recommended: true, spoiler: false },
    ]
    for (const review of demoReviews) await prisma.workReview.upsert({ where: { userId_workId: { userId: review.userId, workId: work.id } }, update: { ...review, workId: work.id, status: 'published' }, create: { ...review, workId: work.id, status: 'published' } })
  }
